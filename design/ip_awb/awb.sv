...
always_ff@(posedge clk or negedge rstn) begin   // state: 0-statistic, 1-awb calc
    if(~rstn)
        ...
        state <= (h_cnt==H-1 && v_cnt==V-1 && pixel_data_in_vld)?    ~state : state;
    else
        state <= 1'b0;
end
...
assign r_avg = state?   r_sum/(H-1*V-1) : 1;
...
assign g_avg = state?   g_sum/(H-1*V-1) : 1;
...
assign b_avg = state?   b_sum/(H-1*V-1) : 1;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        awb_done <= 1'b0;
    else if(awb_en && state && pixel_data_in_vld && (v_cnt==V-1 && h_cnt==H-1))
        awb_done <= 1'b1;
    else 
        awb_done <= 1'b0;
end
