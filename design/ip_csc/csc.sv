...
logic signed [DW/3+DW+2:0] pixel_data_out_r_tmp;
logic signed [DW/3+DW+2:0] pixel_data_out_g_tmp;
logic signed [DW/3+DW+2:0] pixel_data_out_b_tmp;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        h_cnt <= 'd0;
    else if(csc_en && pixel_data_in_vld)
        h_cnt <= (h_cnt==H-1)?  'd0:(h_cnt+1'b1);
end
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        v_cnt <= 'd0;
    else if(csc_en && pixel_data_in_vld && h_cnt==H-1)
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        csc_done <= 1'b0;
    else if(csc_en && v_cnt==V-1 && h_cnt==H-1)
        csc_done <= 1'b1;
    else if(csc_done)
        csc_done <= 1'b0;
end
...
integer file_csc;
initial begin
    file_csc = $fopen("./csc_result.csv","w+");  // 初始化文件
end
integer x;
integer y;
