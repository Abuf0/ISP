...
blc_bias = [0,0,0,0]
blc_clip = 255 #255
awb_parameter = [1.1,0.9,0.9,1.0]
awb_clip = 255 #255
cnf_thres = 0
cnf_parameter = awb_parameter
cnf_clip = 255 #255
cfa_clip = 255 #255
ccm = np.zeros((3,4))  
ccm[0][0] = 1024 # b
...
obj = DPC(raw_data,dpc_thres,'mean',dpc_clip)
...
obj = BLC(dpc_data,blc_bias,bayer_type,blc_clip)
...
##parameter = [1,1,1,1]
#clip = 250
obj = AWB(aaf_data,awb_parameter,bayer_type,awb_clip)  
...
obj = CNF(awb_data,bayer_type,cnf_thres,cnf_parameter,cnf_clip)
...
#clip=250
obj = CFA(cnf_data,'malvar',bayer_type,cfa_clip)
#obj = CFA(raw_data,'malvar',bayer_type,cfa_clip)    ######### TODO for debug
...
maxval = pow(2,gac_bw)
ind = range(0,maxval)
val = [round(pow(float(i)/maxval,gac_gamma)*maxval) for i in ind]
...
f.close()
csc_data_rgb = cv2.cvtColor(csc_data, cv2.COLOR_YCrCb2BGR)
cv2.imwrite('./pipeline_data/img_csc.jpg',csc_data_rgb)
...
obj = NLM(csc_data[:,:,0],1,3,nlm_h,nlm_clip,nlm_lut_en,lut_exp)
#obj = NLM(gc_data[:,:,0],1,3,nlm_h,nlm_clip,nlm_lut_en,lut_exp) # debug
...
obj = BCC(ee_data, bcc_brightness, bcc_contrast, bcc_clip)
...
bcc_data_3 = csc_data
bcc_data_3[:,:,0] = bcc_data
bcc_rgb = cv2.cvtColor(bcc_data_3, cv2.COLOR_YCrCb2BGR)
cv2.imwrite('./pipeline_data/img_bcc.jpg',bcc_rgb)
...
bcc_fcs_rgb = cv2.cvtColor(bcc_fcs_data, cv2.COLOR_YCrCb2BGR)
cv2.imwrite('./pipeline_data/img_bcc_fcs.jpg',bcc_fcs_rgb)
...
obj = HSC(fcs_data,hsc_hue,hsc_saturation,hsc_clip)
...
yum_out[:,:,0] = bcc_data
yum_out[:,:,1:3] = fcs_data
#yum_out[:,:,0] = bcc_data
#yum_out[:,:,1:3] = csc_data_bk[:,:,1:3]
for y in range(raw_h):
    for x in range(raw_w):
...
yuv_out_rgb = cv2.cvtColor(yum_out, cv2.COLOR_YCrCb2BGR)
cv2.imwrite('./pipeline_data/img_yuw_out.jpg',yuv_out_rgb)
